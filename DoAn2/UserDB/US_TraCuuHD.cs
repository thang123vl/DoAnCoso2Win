...
{
    public partial class US_TraCuuHD : UserControl
    {
        ...
        private void LoadAllInvoices()
        {
            ...
            // Tạo đối tượng Functions để gọi hàm GetData
            Functions functions = new Functions();
            DataSet ds = functions.GetData(query, null);

            // Kiểm tra xem có dữ liệu không và gán vào DataGridView
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                dgvHDBanHang.DataSource = ds.Tables[0]; // Gán dữ liệu vào DataGridView
            }
            ...
            {
                dgvHDBanHang.DataSource = null; // Nếu không có dữ liệu, xóa nội dung trong DataGridView
                MessageBox.Show("Không có hóa đơn nào để hiển thị.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }

        private void US_TraCuuHD_Load(object sender, EventArgs e)
        {
            LoadAllInvoices(); // Tải tất cả hóa đơn khi form được mở
        }
    }
}
